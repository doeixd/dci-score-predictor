#!/usr/bin/env node
// Run a DCI score prediction from a season-data JSON file and print a ranked
// recap table plus readiness tiers and caveats.
//
// Usage: node predict.mjs <season-data.json> [--members N] [--explain] [--strict]
//
...
if (!file) {
  console.error('usage: node predict.mjs <season-data.json> [--members N] [--explain] [--strict]');
  process.exit(2);
}
...
try {
  ...
  console.error(`Could not load "${modName}". Install dci-score-predictor in this project first.`);
...
console.log(`${pad('#', 3)}${pad('Corps', 26)}${pad('Total', 9)}${pad('GE', 8)}${pad('Vis', 8)}${pad('Mus', 8)}${pad('Tier', 12)}`);
console.log('-'.repeat(75));
for (const p of result.predictions) {
  const r = tierByKey.get(p.corpsKey);
  console.log(`${pad(p.rank, 3)}${pad(p.corps.slice(0, 24), 26)}${num(p.total)}  ${num(p.GE)} ${num(p.Visual)} ${num(p.Music)}  ${pad(r ? `${r.tier} (${r.tierCode})` : '?', 12)}`);
}
...
if (result.explain) {
  console.log('\nExplain (per corps): baseline/trend/offsets attached in result.explain[]');
}
