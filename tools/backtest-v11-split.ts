// V11 OVERFIT-AUDIT split backtest (maintainers only). VARIANT of backtest-v11.ts.
// Same leakage-safe per-event backtest, but buckets every target event by date
// into: in-sample (<=2026-07-11, both families trained on it), post-cutoff
// (2026-07-12..19, in the contract snapshot but AFTER the 07-11 train cutoff),
// and true-held-out (>=2026-07-20, scored AFTER the contract snapshot — pulled
// live from the prod DB). Runs ONLY the two pure pools (v104, v11) in ONLY the
// agnostic (production-default) mode, and emits per-observation errors so a
// memorization histogram can be drawn. Answers: does v11's relative edge persist
// on rows neither family could have memorized?
//
...
// Train cutoff: both v10.4 + v11 trained on data <= 2026-07-11.
const TRAIN_CUTOFF = '2026-07-11';
// Last date present in the contract snapshot (/tmp/sdk-assets-contract.db).
const CONTRACT_MAX_DATE = '2026-07-19';
const bucketFor = (d: string): 'in' | 'post' | 'held' =>
  d <= TRAIN_CUTOFF ? 'in' : d <= CONTRACT_MAX_DATE ? 'post' : 'held';
...
// ── Inject TRUE-HELD-OUT events (scored AFTER the contract snapshot) ──────────
// The contract DB stops at CONTRACT_MAX_DATE. Any 2026 WC/OC events scored later
// exist only in the prod DB — pull them and append to perfRows in the SAME shape.
// These are TARGET-ONLY (latest events), so only corps_key/division/total_score
// are read (leakage-safe: model inputs still come from prior contract shows).
// percent_through = (date - seasonStart)/(seasonEnd - seasonStart)*100, verified
// byte-exact against contract rows (2026-07-19 -> 23/43 -> 53.4884).
const daysBetween = (a: string, b: string): number =>
  Math.round((Date.parse(b) - Date.parse(a)) / 86400000);
const spanDays = daysBetween(seasonBounds.start, seasonBounds.end);
const heldRows = q(DB, `
  SELECT e.slug AS slug, substr(e.start_date,1,10) AS date,
         cs.corps_key AS corps_key, cs.division_name AS division_name,
         cs.total_score AS total_score
  FROM corps_scores cs
  JOIN events e ON e.slug = cs.competition_slug
  JOIN competitions c ON c.slug = e.slug
  WHERE c.season = '2026'
    AND substr(e.start_date,1,10) > '${CONTRACT_MAX_DATE}'
    AND substr(e.start_date,1,10) <= '${WINDOW_END}'
    AND cs.division_name IN ('World Class','Open Class')
`).map((r: any) => ({
  slug: r.slug,
  date: r.date,
  percent_through: (daysBetween(seasonBounds.start, r.date) / spanDays) * 100,
  corps_key: r.corps_key,
  division_name: r.division_name,
  total_score: r.total_score,
  GE1: null, GE2: null, VP: null, VA: null, CG: null, MB: null, MA: null, MP: null,
}));
console.error(`[held-out] injected ${heldRows.length} obs from prod DB across ${new Set(heldRows.map((r: any) => r.slug)).size} events (> ${CONTRACT_MAX_DATE})`);
perfRows.push(...heldRows);
...
interface Obs { pool: string; bucket: string; slug: string; date: string; corpsKey: string; tier: string; division: string; pred: number; actual: number; err: number; absErr: number }
const perObs: Obs[] = [];
...
async function runEvent(
  ...
  for (const p of result.predictions) {
    ...
    perObs.push({ pool: poolKey, bucket, slug, date, corpsKey: p.corpsKey, tier, division: p.division, pred: p.total, actual, err, absErr: Math.abs(err) });
    evaluated++;
  }
  return evaluated;
}
...
const BUCKETS = ['in', 'post', 'held'] as const;
const BUCKET_LABEL: Record<string, string> = {
  in: 'in-sample (<=07-11)',
  post: 'post-cutoff (07-12..19)',
  held: 'true-held-out (>=07-20)',
};
...
async function backtest() {
  // buckets for reporting the event membership
  const evtByBucket = new Map<string, Set<string>>(BUCKETS.map((b) => [b, new Set()]));
  for (const slug of targets) evtByBucket.get(bucketFor(slugDate.get(slug)!))!.add(slug);
  console.log(`V11 SPLIT backtest ${WINDOW_START}..${WINDOW_END}: ${targets.length} events, ${POOLS.length} pools, agnostic only`);
  for (const b of BUCKETS) console.log(`  ${BUCKET_LABEL[b]}: ${evtByBucket.get(b)!.size} events`);

  // passes[poolKey][bucketKey]
  const passes = new Map<string, Map<string, PassResult>>(
    POOLS.map((p) => [p.key, new Map(BUCKETS.map((b) => [b, mkPass()]))])
  );
  ...
  for (const pool of POOLS) {
    ...
    for (const slug of targets) {
      const bucket = bucketFor(slugDate.get(slug)!);
      try {
        await runEvent(pool.key, slug, passes.get(pool.key)!.get(bucket)!, 'agnostic', provider);
        evtOk++;
      } catch (e) {
  ...
  for (const b of BUCKETS) {
    lines.push(`\n============ BUCKET ${BUCKET_LABEL[b]} (${evtByBucket.get(b)!.size} events) ============`);
    for (const pool of POOLS) {
      const pass = passes.get(pool.key)!.get(b)!;
      lines.push(`--- pool ${pool.key} (${pool.label}) — (n | MAE | bias) ---`);
    ...
    // relative edge line
    const v104 = passes.get('v104')!.get(b)!.overall;
    const v11 = passes.get('v11')!.get(b)!.overall;
    const rel = ((mae(v11) - mae(v104)) / mae(v104)) * 100;
    lines.push(`  >>> v11 vs v10.4 overall: ${mae(v104).toFixed(3)} -> ${mae(v11).toFixed(3)}  (${rel >= 0 ? '+' : ''}${rel.toFixed(1)}%)`);
  }
  const report = lines.join('\n');
  console.log(report);

  // ── Check 3: memorization histogram — per-obs |err| quantiles per bucket/pool ──
  const histLines: string[] = ['\n===== |err| distribution (memorization check) ====='];
  for (const b of BUCKETS) {
    for (const pool of POOLS) {
      const errs = perObs.filter((o) => o.bucket === b && o.pool === pool.key).map((o) => o.absErr).sort((x, y) => x - y);
      if (!errs.length) continue;
      const near0 = errs.filter((e) => e < 0.5).length;
      histLines.push(`  ${b.padEnd(5)} ${pool.key.padEnd(5)} n=${String(errs.length).padStart(3)}  p10=${quantile(errs,0.1).toFixed(2)} p25=${quantile(errs,0.25).toFixed(2)} p50=${quantile(errs,0.5).toFixed(2)} p75=${quantile(errs,0.75).toFixed(2)} p90=${quantile(errs,0.9).toFixed(2)} min=${errs[0]!.toFixed(2)}  <0.5:${near0}(${(100*near0/errs.length).toFixed(0)}%)`);
    }
  }
  const histReport = histLines.join('\n');
  console.log(histReport);

  const json = {
    window: { start: WINDOW_START, end: WINDOW_END },
    trainCutoff: TRAIN_CUTOFF,
    contractMaxDate: CONTRACT_MAX_DATE,
    mode: 'agnostic',
    skips,
    buckets: Object.fromEntries(BUCKETS.map((b) => [b, {
      label: BUCKET_LABEL[b],
      events: [...evtByBucket.get(b)!].sort(),
      pools: Object.fromEntries(POOLS.map((pool) => {
        ...
        return [pool.key, {
          ...
          absErrQuantiles: (() => {
            const errs = perObs.filter((o) => o.bucket === b && o.pool === pool.key).map((o) => o.absErr).sort((x, y) => x - y);
            return errs.length ? { n: errs.length, p10: quantile(errs,0.1), p25: quantile(errs,0.25), p50: quantile(errs,0.5), p75: quantile(errs,0.75), p90: quantile(errs,0.9), min: errs[0], under0_5: errs.filter((e) => e < 0.5).length } : null;
          })(),
        }];
      })),
    }])),
  ...
  fs.writeFileSync(path.join(HERE, 'backtest-v11-split.out.json'), JSON.stringify(json, null, 2));
  console.log(`\nWrote ${path.join(HERE, 'backtest-v11-split.out.json')}`);
}
