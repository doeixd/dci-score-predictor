...
- The in-repo `data-2026/` companion package (2026 only, never published to npm)
  ...
  Data-only users can still `npm install dci-season-data`. Clone with
  `--recursive` to check out the data locally. Generated by
  `tools/gen-all-seasons-data.ts`.

### `dci-score-predictor/data` subpath export

- The bundled seasons are now reachable directly from the SDK:
  `import { season, seasons } from 'dci-score-predictor/data'` (ESM **and**
  `require()`), so consumers no longer need a second install for the common
  case. Backed by the `data/` submodule payload, which is now shipped in the
  npm tarball (`data/index*` + `data/data/*.json`, ~4.7 MB). The subpath is a
  thin typed wrapper (`src/data.ts`) that resolves the season JSON from the
  package root at runtime (fs walk-up, layout-independent).
- **Publisher note:** publishing now requires the git submodule to be
  initialized first, or the tarball ships without season JSON — run
  `git submodule update --init` before `npm publish` / `npm pack`.
